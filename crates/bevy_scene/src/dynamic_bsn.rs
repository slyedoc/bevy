...
use crate::{
    dynamic_bsn_grammar::TopLevelPatchesParser, dynamic_bsn_lexer::Lexer, ErasedComponentTemplate,
    InheritSceneAsset, NameEntityReference, RelatedResolvedScenes, ResolveContext,
...
impl AssetLoader for DynamicBsnLoader {
    ...
    async fn load(
        ...
        Ok(ScenePatch {
            scene: Some(Box::new(SceneScope(
                patch.scene.expect("convert_bsn_patches_to_patch produced empty scene"),
            ))),
    ...
    fn convert_bsn_patch_to_patch(
        ...
        let patch = match *patch {
            ...
            BsnPatch::Relation(BsnRelation(ref relation_symbol, ref patches)) => {
                ...
                let related_template_list: Vec<Box<dyn Scene>> =
                    related_template_list.into_iter().flatten().collect();
...
impl Scene for MultiPatch {
    ...
    fn register_dependencies(&self, dependencies: &mut SceneDependencies) {
        for subscene in self.0.iter() {
            subscene.register_dependencies_box(dependencies);
        }
    }
}
...
impl Scene for DynamicRelatedScenes {
    fn resolve(
        ...
        let related = scene
            .related
            .entry(self.relationship)
            .or_insert_with(RelatedResolvedScenes::new::<ChildOf>);
    ...
    fn register_dependencies(&self, dependencies: &mut SceneDependencies) {
        for scene in self.related_template_list.iter() {
            scene.register_dependencies_box(dependencies);
        }
    }
}
