...
pub fn init_allocator(
    ...
    // and is REQUIRED: the RT pipeline is layout-free (descriptor-heap
    // mappings), with no descriptor-set path. Without the extension, solari
    // disables wholesale — same as the cluster-AS/OMM/RT-pipeline gate above.
    let Some(seam) = crate::gpu::binding_seam::BindingSeam::try_new(&memory) else {
        bevy_log::warn_once!(
            "bevy_solari disabled: the driver lacks VK_EXT_descriptor_heap \
             (NVIDIA R610+). All solari passes no-op."
        );
        return;
    };
    commands.insert_resource(seam);
