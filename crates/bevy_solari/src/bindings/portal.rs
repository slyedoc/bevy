...
//! from the paired portal instead: into portal-local space, a half-turn
//! about local Y, out through the target's frame — so looking INTO this
//! surface shows the view OUT of the target's front (+Z). Pair two portals
//! by pointing their `target`s at each other; a one-way portal is just an
//! unpaired one.
//!
//! The GPU table is a [`gpu_table!`](crate::gpu_table) column of instance-
//! slot PAIRS, slot-indexed by `GpuSlot<SolariPortals>`; the ray map derives
//! at hit time from the live GPU transform column, so portals on moving or
//! GPU-propagated parents stay exact with zero CPU transform reads. Because
//! the redirect happens during ray traversal (`portal_redirect` in
//! `raytracing_scene_bindings.wgsl`), recursion is free: portals seen
//! through portals, portals in reflections, portals through glass. Light is
//! NOT transported — portals carry the view, not next-event estimation, so
//! each side is lit by its own surroundings.
...
crate::gpu_table! {
    ...
    /// (portal instance, target instance) pairing the traversal scan matches
    /// hits against.
