...
use bevy_render::{
    ...
    render_resource::{
        ...
        BufferDescriptor, BufferUsages, ComputePassDescriptor, PipelineCache, RawBufferVec,
...
fn pool_buffer(device: &RenderDevice, label: &'static str, bytes: u64) -> Buffer {
...
/// `RenderStartup`: build the deform pool buffers. The bind-group layout lives in
/// `SolariResourceManager`, the pipeline id in `SolariPipelines`.
pub fn init_deform(mut commands: Commands, render_device: Res<RenderDevice>) {
    let n = (MAX_ANIMATED_INSTANCES * MAX_VERTS_PER_ANIMATED_MESH) as u64;
    let positions = pool_buffer(&render_device, "deform.positions", n * 12);
    let normals = pool_buffer(&render_device, "deform.normals", n * 4);
    let tangents = pool_buffer(&render_device, "deform.tangents", n * 16);
    let animated_table = make_animated_table(&render_device, 1);
...
fn make_animated_table(device: &RenderDevice, slots: u32) -> Buffer {
    device.create_buffer(&BufferDescriptor {
        label: Some("deform.animated_table"),
        size: slots.max(1) as u64 * size_of::<GpuAnimatedInstance>() as u64,
...
pub fn prepare_deform(
    ...
    *deform.params.get_mut() = DeformParams {
    ...
    if high_water > deform.animated_table_capacity {
        ...
        deform.animated_table = make_animated_table(&render_device, deform.animated_table_capacity);
