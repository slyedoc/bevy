...
// See `cluster::gpu_instances::GpuColumn`.
...
struct ScatterParams {
    ...
    words_per_value: u32,
    // 1 on a grown buffer (history columns): set `previous = new` instead of
    // shifting the (garbage) current value in.
    force_init: u32,
    _pad1: u32,
}
...
// overwriting it. The GPU already holds last frame's value — uploading a
...
fn scatter_with_history(
    ...
    for (var w = 0u; w < params.words_per_value; w = w + 1u) {
        ...
        previous[dst + w] = select(column[dst + w], new_word, params.force_init != 0u);
