...
pub fn embed_solari_shaders(app: &mut App) {
    ...
    // restir_spatial.wgsl (a wgpu compute pass) #imports rt_payload.wgsl's
    // RtCamera struct — the wgpu pipeline cache resolves that only for
    // registered libraries. (The RT stages themselves are Slang and use the
    // rt_payload.slang mirror.)
