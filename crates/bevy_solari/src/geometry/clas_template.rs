...
/// `upload_pending_clas` consumes the queue): drain
/// [`ClusterMeshManager::pending_template_uploads`] and build templates for
/// each freshly-uploaded animated mesh.
pub fn upload_pending_templates(
    ...
    let (Some(allocator), Some(fns), Some(mut arena)) = (allocator, fns, arena) else {
        cluster_meshes.pending_template_uploads.clear();
        return;
    };
    if fns.cluster.is_none() || cluster_meshes.pending_template_uploads.is_empty() {
        cluster_meshes.pending_template_uploads.clear();
        return;
    }
    ...
    let pending = core::mem::take(&mut cluster_meshes.pending_template_uploads);
