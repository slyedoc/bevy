...
/// The build inputs + scratch are transients; [`Self::split`] hands them to
...
impl MicromapBuild {
    /// Split into the persistent [`MeshOmm`] and the build transients the
    /// caller MUST retire (the build submit references them by address).
    fn split(self) -> (MeshOmm, (wgpu::Buffer, wgpu::Buffer, wgpu::Buffer)) {
        (
            MeshOmm {
...
impl ClasArena {
    ...
    pub fn upload_mesh(
        ...
        // Retire every buffer the raw builds reference by device address —
        // wgpu can't see those uses, so the reaper owns their destruction.
        let omm = omm_build.map(|b| {
            let (mesh_omm, omm_transients) = b.split();
            retire.retire(render_queue, "clas_arena.omm_transients", omm_transients);
            mesh_omm
        });
        self.meshes.insert(
            asset_id,
        ...
        retire.retire(
            render_queue,
            "clas_arena.upload_transients",
            (src_infos_buf, count_buf, scratch_buf, dst_addresses_buf),
        );
    }
}
