...
use crate::entry::{clear_children, entity_component_sections, resource_section};
...
impl<F: QueryFilter + Send + Sync + 'static> Plugin for WorldInspectorPlugin<F> {
    fn build(&self, app: &mut bevy_app::App) {
        ...
        app.add_systems(Update, (toggle_open, world_inspector_rebuild::<F>).chain());
    }
}
...
fn build_tree(world: &mut World, root: Entity, entities: &[Entity]) {
    ...
    for type_id in reflectable_resources(world, &registry) {
        ...
        if expanded && let Some(section) = resource_section(world, &registry, type_id) {
            children.push(indent(section));
        }
    }
