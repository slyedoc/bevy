...
use bevy_ui::Checked;
use bevy_ui_widgets::{checkbox_self_update, slider_self_update, SliderPrecision, SliderValue};
...
///
/// The `build` function is monomorphized per type at registration time (see [`CreateTypeData`]), so it
...
macro_rules! impl_numeric_widget {
    ($t:ty) => {
        impl CreateTypeData<$t> for ReflectInspectorWidget {
...
impl CreateTypeData<bool> for ReflectInspectorWidget {
    fn create_type_data(_: ()) -> Self {
