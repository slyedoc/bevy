...
///   sync with the parent's plane and [`ColorPlaneValue`] — the WGSL shader draws the gradient;
...
fn update_plane_thumb(
    ...
    for (plane_ent, plane_value) in q_color_plane.iter() {
        ...
        let Some(inner_ent) = q_children.get(plane_ent).ok().and_then(|c| c.first().copied())
        else {
            continue;
        };
        let Some(thumb_ent) = q_children.get(inner_ent).ok().and_then(|c| c.first().copied())
        else {
            continue;
        };
...
impl Plugin for ColorPlanePlugin {
    fn build(&self, app: &mut bevy_app::App) {
        app.add_systems(PostUpdate, update_plane_thumb);
