...
impl ComponentSparseSet {
    ...
    pub(crate) unsafe fn insert(
        ...
        if let Some(&dense_index) = self.sparse.get(entity.index()) {
            ...
            self.dense.push(value, ComponentTicks::new(change_tick), caller);
            self.sparse
    ...
    /// Returns `None` if `entity` does not have a component in the sparse set.
    #[inline]
    pub fn get_with_ticks(&self, entity: Entity) -> Option<(Ptr<'_>, TickCells<'_>, &UnsafeCell<core::panic::Location<'static>>,)> {
