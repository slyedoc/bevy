...
impl<const SEND: bool> ResourceData<SEND> {
    ...
    #[inline]
    pub(crate) fn get_with_ticks(&self) -> Option<(Ptr<'_>, TickCells<'_>, &UnsafeCell<core::panic::Location<'static>>,)> {
        self.is_present().then(|| {
            self.validate_access();
            (
                // SAFETY: We've already checked if a value is present, and there should only be one.
    ...
    pub(crate) fn remove(&mut self) -> Option<(OwningPtr<'_>, ComponentTicks, core::panic::Location<'static>,)> {
        if !self.is_present() {
            return None;
        }
        if !SEND {
