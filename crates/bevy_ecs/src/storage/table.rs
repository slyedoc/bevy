...
impl Column {
    ...
    /// Assumes data has already been allocated for the given row.
    #[inline]
    pub(crate) unsafe fn replace(&mut self, row: TableRow, data: OwningPtr<'_>, change_tick: Tick, caller: core::panic::Location<'static>) {
        debug_assert!(row.as_usize() < self.len());
    ...
    pub(crate) unsafe fn swap_remove_and_forget_unchecked(
        &mut self,
        row: TableRow,
    ) -> (OwningPtr<'_>, ComponentTicks, core::panic::Location<'static>,) {
    ...
    pub(crate) unsafe fn push(&mut self, ptr: OwningPtr<'_>, ticks: ComponentTicks, caller: core::panic::Location<'static>,) {
        self.data.push(ptr);
    ...
    #[inline]
    pub fn get(&self, row: TableRow) -> Option<(Ptr<'_>, TickCells<'_>, &UnsafeCell<core::panic::Location<'static>>)> {
...
impl Table {
    ...
    pub(crate) unsafe fn allocate(&mut self, entity: Entity) -> TableRow {
        ...
        for column in self.columns.values_mut() {
            ...
            column.callers.push(UnsafeCell::new(*core::panic::Location::caller()));
        }
