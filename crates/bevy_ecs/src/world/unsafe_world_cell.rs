...
impl<'w> UnsafeWorldCell<'w> {
    ...
    pub(crate) unsafe fn get_resource_with_ticks(
        self,
        component_id: ComponentId,
    ) -> Option<(Ptr<'w>, TickCells<'w>,  &'w UnsafeCell<core::panic::Location<'static>>,)> {
        // SAFETY:
...
unsafe fn get_component_and_ticks(
    ...
    entity: Entity,
    location: EntityLocation,
) -> Option<(Ptr<'_>, TickCells<'_>,    &UnsafeCell<core::panic::Location<'static>>,)> {
    match storage_type {
        StorageType::Table => {
            let components = world.fetch_table(location, component_id)?;
